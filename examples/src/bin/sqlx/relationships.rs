...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = db.execute_raw("DROP TABLE IF EXISTS books CASCADE", vec![]).await;
    let _ = db.execute_raw("DROP TABLE IF EXISTS authors CASCADE", vec![]).await;
    db.execute_raw(
    ...
    let king = Author { name: "Stephen King".into(), ..Default::default() }.create(&db).await?;
    
    Book { title: "The Shining".into(), author_id: king.id, ..Default::default() }.create(&db).await?;
    Book { title: "IT".into(), author_id: king.id, ..Default::default() }.create(&db).await?;
    Book { title: "Misery".into(), author_id: king.id, ..Default::default() }.create(&db).await?;
